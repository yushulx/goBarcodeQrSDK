...
# Builds the becho -e "\033[1;33mBuilding goBarcodeQrSDK for Linux\033[0m"
...
echo -e "\033[1;33m🚀 Building goBarcodeQrSDK for Linux\033[0m"
...
if ! command -v cmake &> /dev/null; then
    write_error "CMake not found. Please install cmake: sudo apt-get install cmake"
    exit 1
fi

if ! command -v go &> /dev/null; then
    write_error "Go not found. Please install Go from https://golang.org/dl/"
    exit 1
fi

if ! command -v gcc &> /dev/null; then
    write_error "GCC not found. Please install build essentials: sudo apt-get install build-essential"
    exit 1
fi
...
BRIDGE_LIB="dcv/lib/linux/libbridge.so"
...
echo -e "  - Bridge library: dcv/lib/linux/libbridge.so"
